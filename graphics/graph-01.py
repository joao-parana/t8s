...
if 'counter1' not in session_state:
    session_state.counter1 = 0
if 'total1' not in session_state:
    session_state.total1 = 0
if 'counter2' not in session_state:
    session_state.counter2 = 0
if 'total2' not in session_state:
    session_state.total2 = 0
if 'counter3' not in session_state:
    session_state.counter3 = 0
if 'total3' not in session_state:
    session_state.total3 = 0
...
option_selected = (
    st.radio(
        'Escolha dentre os 3 exemplos abaixo:',
        [
...
print(
    f'option_selected = {option_selected}, type(option_selected) = {type(option_selected)}'
)
...
def create_df_with_nans_and_mask() -> pd.DataFrame:
    ...
    p = len(values_a)
    df = pd.DataFrame(
        {'t': pd.date_range('2023-07-26', periods=p, freq='D'), 'a': values_a}
    )
...
match option_selected.split(' - ')[0]:
    ...
    case "2":
        ...
        def build_chart_2(cached: int):
            ...
            st._arrow_line_chart(other_data, x='TIMESTAMP', y=['T6021', 'T6023'])
    ...
    case "3":
        ...
        def build_chart_3(cached: int):
            ...
            chart = (
                alt.Chart(other_data)
                .transform_fold(['T6021', 'T6023'], as_=['Sensor', 'Measurement'])
