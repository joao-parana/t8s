...
@then(
    'I can list the context attributes in another step and check the created attribute'
)
def step_impl_2(context):
    ...
    assert callable(
        context.minha_funcao_1
    ), "A função minha_funcao_1 não foi criada corretamente"
    assert isinstance(
        context.minha_funcao_2, type(lambda x: x)
    ), 'context.minha_funcao is not a function'
    assert callable(
        context.minha_funcao_2
    ), "A função minha_funcao_2 não foi criada corretamente"
...
def final_table(context):
    ...
    logger.info(
        f'STEP: Then the last step has a final table -> headings: {headings}, {type(headings)}'
    )
    logger.info(
        f'STEP: Then the last step has a final table -> rows:{rows}, {type(rows)}'
    )
    for idx, row in enumerate(table.rows):
        ...
        logger.info(
            f'STEP: Then the last step has a final table -> '
